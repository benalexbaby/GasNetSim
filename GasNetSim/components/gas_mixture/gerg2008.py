...
class GasMixtureGERG2008:
    def __init__(self, P, T, x, pressure_unit='Pa', temperature_unit='K'):
        # Input parameters
        if pressure_unit == 'Pa':
        ...
        self.T = T
        self.x = x  # gas composition

        # Calculated properties
        self.MolarMass = self.MolarMassGERG()
        self.MolarDensity = 0
        self.rho = self.DensityGERG(iFlag=0)
        self.PropertiesGERG()
        self.SG = 1
        self.Z = 1
        self.energy = 0
    ...
    def DensityGERG(self, iFlag=0):
        ...
        # nFail = 0
        # iFail = 0
        # plog = vlog = P2 = Z = dpdlv = vdiff = tolr = vinc = 0.0
        D = 0  # initial estimate of the density
        Tcx = Dcx = 0.0

        dPdD = d2PdD2 = d2PdTD = dPdT = U = H = S = A = 0.0
        Cv = Cp = W = G = JT = Kappa = PP = 0.0
        ...
        for it in range(1, 51):
            if (vlog < -7) or (vlog > 100) or (it == 20) or (it == 30) or (it == 40) or (iFail == 1):
                ...
                vlog = -math.log(D)
            D = math.exp(-vlog)
            P2, Z, dPdDsave = self.PressureGERG(T, D, x)
...
def ReducingParametersGERG(x, Tr, Dr):
    """
...
def AlpharGERG(itau, idelta, T, D, x):
    ...
    mn = 0
    Tr = Dr = delta = tau = 0.0
    lntau = ex = ex2 = ex3 = cij0 = eij0 = 0.0
    delp = Expd = np.zeros(7+1)
    ndt = ndtd = ndtt = xijf = 0.0
    ar = np.zeros((4,4))
            ...
            for j in range(i+1, NcGERG+1):  # for (int j = i + 1; j <= NcGERG; ++j)
                if x[j] > epsilon:
                    ...
                    if mn >= 0:
                        ...
                        for k in range(1, kpolij[mn] + 1):  # for (int k = 1; k <= kpolij[mn]; ++k)
                        ...
                        for k in range(1+kpolij[mn], kpolij[mn]+kexpij[mn]+1):  # for (int k = 1 + kpolij[mn]; k <= kpolij[mn] + kexpij[mn]; ++k)
...
def tTermsGERG(lntau, x):
    ...
    for k in range(1, kpol[i] + kexp[i] + 1):  # for (int k = 1; k <= kpol[i] + kexp[i]; ++k)
...
if __name__ == "__main__":
    from typical_mixture_composition import NATURAL_GAS
